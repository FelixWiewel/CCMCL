...
RUNS = 5
LOG_PATH = "../logs/BiC/FashionMNIST"
...
for i, BUFFER_SIZE in enumerate(BUFFER_SIZES):
    for run in range(RUNS):
        ...
        model = models.BiC_CNN(10)
        buf = models.BiCBuffer(BUFFER_SIZE)
        train = utils.Trainer()
        ...
        for t in range(TASKS):
            ...
            # Reset bias correction parameters
            model.w = tf.ones(1, dtype=tf.float32)
            model.b = tf.zeros(1, dtype=tf.float32)
            # Training loop
            ...
            for x, y in train_ds:
                ...
                if t > 0:
                    x_r, y_r, l_r = buf.sample(int(BATCH_SIZE/2))
                    current_loss = train.train_distill_step(x, y, x_r, y_r, l_r, model, optimizer, t)
                else:
                ...
                if iters == ITERS:
                    ...
                    for c in classes:
                        ...
                        for x, y in train_ds:
                            buf.add_samples(x[0:num_add], y[0:num_add], model(x[0:num_add]))
                            break
                    buf.summary()
                    iters = 0
                    break
            ...
            if t > 0:
                ...
                for j in range(ITERS):
                    x_r, y_r, l_r = buf.sample(BATCH_SIZE)
                    loss = train.bias_correction_step(x_r, y_r, model, optimizer)
                    if j % VAL_ITERS == 0:
                        print("Iteration: {} Bias correction loss: {:.3}".format(j, loss))
